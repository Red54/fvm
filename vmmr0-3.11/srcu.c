...
static int vmmr0_rcu_sync_thread(void *data)
{
	int badcpu;
	int cpu = (long)data;
	...
	while (!kthread_should_stop()) {
		...
		if (likely(cpu == badcpu)) {
			...
			req->success = false;
			WARN_ONCE(1, "vmmr0_rcu_sync_thread() on CPU %d, "
				  "expected %d\n", badcpu, cpu);
		}
		preempt_enable();
...
{
	__synchronize_srcu(sp, vmmr0_synchronize_sched_expedited);
}
...
int vmmr0_init_srcu(void)
{
	...
	get_online_cpus();
	for_each_online_cpu(cpu) {
		p = kthread_create(vmmr0_rcu_sync_thread, (void *)(long)cpu,
				   "vmmr0srcusync/%d", cpu);
		if (IS_ERR(p))
			goto error_out;
