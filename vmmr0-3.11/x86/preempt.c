...
static void preempt_enable_sched_out_notifiers(void)
{
	...
	current->thread.vmmr0_compat_debugreg(7) = 0ul;
#ifdef TIF_DEBUG
...
static void preempt_enable_sched_in_notifiers(void * addr)
{
	...
	asm volatile ("mov %0, %%db7" : : "r"(0x701ul));
	current->thread.vmmr0_compat_debugreg(0) = (unsigned long) addr;
	current->thread.vmmr0_compat_debugreg(7) = 0x701ul;
#ifdef TIF_DEBUG
...
void preempt_notifier_sys_init(void)
{
	...
	printk("vmmr0: emulating preempt notifiers;"
	       " do not benchmark on this machine\n");
	dprintk("\n");
	spin_lock_init(&pn_lock);
...
#else
	if (!current->thread.vmmr0_compat_debugreg(7))
#endif
		__preempt_disable_notifiers();
}
...
void preempt_notifier_sys_exit(void)
{
	...
	dprintk("\n");
	vmmr0_on_each_cpu(do_disable, NULL, 1);
	asm ("sidt %0" : "=m"(idt_desc));
