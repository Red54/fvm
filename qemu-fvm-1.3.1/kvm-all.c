...
int kvm_init_vcpu(CPUArchState *env)
{
    ...
    env->kvm_fd = ret;
#ifdef _WIN32
    ENV_GET_CPU(env)->kvm_handle = vmmr3_get_vmmr0_handle();
    ENV_GET_CPU(env)->kick_event = vmmr3_get_kick_event(s, env->kvm_fd);
#endif
    env->kvm_state = s;
    env->kvm_vcpu_dirty = 1;

    env->kvm_run = vmmr3_get_vmmr0_run(s, env->kvm_fd);
...
#ifdef _WIN32
    ret = vmmr3_vcpu_ioctl(env->kvm_fd, ENV_GET_CPU(env)->kvm_handle, type, arg);
#else
    ret = vmmr3_vcpu_ioctl(env->kvm_fd, type, arg);
#endif
    if (ret == -1) {
        ret = -errno;
    }
