...
#elif defined(_WIN32)
#include "kvm.h"
extern CPUArchState * tcg_cpu_single_env;
DECLARE_TLS(CPUArchState *,cpu_single_env);

static inline CPUArchState ** get_cpu_single_env(void)
{
    if(kvm_enabled()) {
        return &tls_var(cpu_single_env);
    } else {
        return &tcg_cpu_single_env;
    }
}

#define cpu_single_env (*get_cpu_single_env())
#else
DECLARE_TLS(CPUArchState *,cpu_single_env);
#define cpu_single_env tls_var(cpu_single_env)
#endif
